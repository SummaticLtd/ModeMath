    ...
    TestList(
        ...
        [   Test.CasesSync(
            ...
            Test.Sync(
                ...
                fun () ->
                    Assert.Equal(1000<du>, MathConstants.UnitsPerEm)
                    Assert.Equal(250<du>, MathConstants.AxisHeight)
                    Assert.Equal(40<du>, MathConstants.FractionRuleThickness)
    ...
    TestList(
        ...
        [   Test.Sync(
            ...
            Test.Sync(
                ...
                fun () ->
                    let x = upright 'x'
                    Assert.True(x.Advance > 0<du>, "advance")
                    Assert.True(x.Top > 0<du>, "top above the baseline")
                    Assert.True(x.Bottom <= 0<du>, "bottom on or below the baseline")
            ...
            Test.Sync(
                ...
                fun () ->
                    ...
                    | ValueSome f -> Assert.True(f.ItalicCorrection > 0<du>, "italic correction")
            )
            Test.Sync(
                "digitsHaveNoItalicCorrection",
                fun () -> Assert.Equal(0<du>, (digit '0').ItalicCorrection)
            )
        ]
    )
                ...
                fun () ->
                    for letter in 'A' .. 'Z' do
                        ...
                        | ValueSome glyph ->
                            ...
                            Assert.True(glyph.Advance > 0<du>, $"{letter} has no advance")
            ...
            Test.Sync(
                ...
                fun () ->
                    for name, accent in
                        ...
                        Assert.Equal(0<du>, accent.Advance, $"{name} takes an advance")
                        Assert.True(accent.TopAccentAttachment < 0<du>, $"{name} attaches right of its origin")
                        Assert.True(accent.Top > 0<du>, $"{name} draws no ink above the baseline")
            )
            Test.Sync(
